# Sprawozdanie z zadania 1 w ramach LABORATORIUM: Programowanie Aplikacji w Chmurze Obliczeniowej

### Zadanie 1

Kod serwera znajdujący się w pliku "serwer.c"
...
// Dodanie html+javascrptu serwera jako stałej w pliku server.c w celu zmniejszenia ostatecznego obrazu.
...
// Funkcja odpowiadająca za logi w trakcie startu serwera
void log_info()
{
    time_t now = time(0);
    if (now == ((time_t)-1))
    ...
    // Użycie fflush() w celu odbuforowania stdout, jako że nie jest to robione automatycznie po znaku nowej ini w obrazie Scratch
...
```Dockerfile
# Stage 1: Kompilacja serwera z wykorzystaniem alpine
...
# Dodanie curl do ostatecznego obrazu aby umozliwić Healthcheck (niestety zwiększa to wielkość obrazu o 0.5 MB)
...
Proponowana nazwa obrazu "weather-server"
Polecenie w celu zbudowania opracowanego obrazu kontenera:

```
Docker build -t weather-server .
```

Polecenie w celu uruchomienia kontenera na podstawie zbudowanego obrazu

```
Docker run -d --name weather-server --rm -p 8080:8080 weather-server
```

Polecenie w celu sposobu uzyskania informacji z logów, które wygenerowałą opracowana aplikacja podczas uruchamiana kontenera:
 - Jeśli na danym obrazie został uruchomiony kontener

```
docker weather-server logs
```
 - Jeśli na danym obrazie nie został uruchomiony kontener:
...
docker images
```
Następnie skopiować ID obrazu (IMAGE_ID) i wykonać polecenie:
```
docker IMAGE_ID logs
```

Polecenie w celu sprawdzenia, ile warstw posiada zbudowany obraz oraz jaki jest rozmiar obrazu.

W celu sprawdzenia wyłącznie rozmiaru obrazu, należy wykonać polecenie:
```
docker images
```

Aby uzyskać bardziej szczegółowe informacje, należy wykonać poniżesze polecenie wykorzystująć ID obrazu (IMAGE_ID):
